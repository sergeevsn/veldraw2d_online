...
def fill_poly():
    if request.method == 'POST':
        ...
        if request.json['shape_type'] == "rectangle":
           x0 = request.json['x0']//step_x
           y0 = request.json['y0']//step_z
           y1 = request.json['y1']//step_z
           x1 = request.json['x1']//step_x
           ...
           cv2.fillPoly(vel_array, [np.array([[x0, y0], [x0, y1], [x1, y1], [x1, y0]], dtype=np.int32)], shape_vel)                     
        ...
        if request.json['shape_type'] == "point":
            radius = request.json['radius']//min([step_x, step_z])
            if radius == 0:
                radius = 1
            pointX = request.json['pointX']
            pointZ = request.json['pointZ']
            cv2.circle(vel_array, (pointX-1, pointZ-1), radius, shape_vel, -1)           
